...
class Analyzer {
    constructor(config) {
        ...
        info("total pages : ", this._meta.glob.length);
    }
    ...
    populateTree(documents_dir) {
        ...
        files.forEach(function (file) {
            let current = tree;
            let file_meta = utils.fileMeta(file, config, config.documents_dir);
    ...
    populateTaxonomyIndex(taxonomy) {
        ...
        taxonomy.forEach(function (item) {
            // main taxonomy type eg. tags, categories etc
            let tax_item_tree = taxonomy_obj[item] = {};
            glob.forEach(function (file) {
                //get the items for each taxonomy
                var clean_file_path = utils.removeBase(base_path, file);
                ...
                if (jp.has(tree, check)) {
                    ...
                    tItem.forEach(function (subtItem) {
                        let tItem_path = "/" + subtItem.toLowerCase();
                        var clean_file_path = utils.removeBase(base_path, file);
    ...
    populatePageIndex(config) {
        ...
        let tree = this._meta.tree;
        let _indexes = {};
        let dir_config = config.pagination.dir;
        let type_config = dir_config['_default_'];

        Object.keys(tree).forEach(function (key) {
            if (!tree[key]["__type__"]) {
                type_config = dir_config.hasOwnProperty(key) ? dir_config[key] : dir_config['_default_'];
                _indexes[key] = createIndex(tree[key], type_config.paginate);
            }
        });
        ...
        function createIndex(obj, paginate = false) {
            let local_indexes = {};
            ...
            local_indexes["index"] = paginate ? arrayToObject(index_array, config.pagination.limit || 10) : index_array;
            Object.keys(obj).forEach(function (key) {
                if (!obj[key]["__type__"])
                    local_indexes[key] = createIndex(obj[key], paginate);
            })
            return local_indexes;
        }

        function indexList(obj) {
            // NOTE: Proxy function is to avaoid stack overflow in JS
