...
class Renderer {
    ...
    render() {
        ...
        function renderFile(file_obj, index = null) {
            ...
            mkdirp(build_path);
            fs.writeFileSync.apply(fs, [path.join(build_path, 'index.html'), html]); 
            
        }

        function renderDir(dir_obj, name) {
            let current_index = jp.get(meta.indexes, utils.normalizePathForJP(name));
            ...
            Object.keys(dir_obj).forEach(function(key) {
                ...
                if(dir_obj[key].hasOwnProperty('__type__')){
                    ...
                    renderDir(dir_obj[key], current_path)
                }
            })
        }
