...
class Renderer {
    ...
    render() {
        ...
        function renderSitemap() {
            ...
            let buld_dir_glob = glob.sync(path.join(config.build_dir, '**/*'));
            ...
            buld_dir_glob.forEach(function(file_path) {
                ...
                let uri = file_path.replace(config.build_path, '');
                site_map_body += `<url>\n\t<loc>${config.site.base_domain}${uri}/</loc>\n\t<lastmod>${lastmod}</lastmod>\n</url>`
            })
            ...
            if(Array.isArray(current_index.index)) {
                ...
                Object.keys(current_index.index).forEach(function(key, key_index, arr) {
                    ...
                    if(!(key_index - 1 < 0)) {
                        ...
                        // paginate['previous_page_url'] = false;
